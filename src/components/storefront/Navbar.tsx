      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex items-center justify-between h-16 gap-4">
          ...
          <div className="flex items-center space-x-3 shrink-0">
            <Link
              href="/cart"
              className="relative flex items-center gap-2 px-3.5 py-2 bg-primary-container text-white rounded-lg font-medium text-sm hover:bg-primary transition-colors shadow-sm"
            >
              <ShoppingBag className="w-4 h-4" />
              <span className="hidden sm:inline font-semibold">Order Cart</span>
              {totalLineItems > 0 && (
                <span className="ml-1 bg-secondary text-white text-xs font-mono font-bold px-2 py-0.5 rounded-full">
                  {totalUnitsCount}
                </span>
              )}
            </Link>
